...
検査データを外部のAIサービスに送信せず、OCRと推論をローカルで完結させる構成
(Dify + Ollama + FastAPI)。データが外に出るのは、利用者自身のGoogle Driveへ
バックアップする場合のみで、これは任意の機能。
...
定期的に血液検査を受けていると、その都度封筒サイズの検査報告書を受け取る。
...
「検査結果AI表示」ワークフローが生成する推移グラフと所見コメント。

![検査結果の推移グラフ](docs/sample_trend.png)

> 尿素窒素とクレアチニンの値は基準値の上限を超えていますが、対象者の状態によっては
> 想定内の範囲として扱われる項目です。両者とも前回からは低下しています。
>
> カリウムの量は範囲内ですが、前回より減っているようです。
>
> リンの量は少し低めで、前回からさらに下がっています。
>
> カルシウムの量は範囲内で安定していますが、アルブミンの影響を考慮する必要があるため、
> 補正後の値も確認が必要です。
>
> ヘモグロビンとヘマトクリットの値は目標より少し低く、前回から減少しています。
> これらは赤血球に関連しており、酸素を効率的に運ぶ能力が低下している可能性があります。
>
> 副甲状腺ホルモン(PTH)の量は範囲内ですが、前回よりも上昇しています。
> これは骨やカルシウムの代謝に影響を与えます。
>
> アルブミンの値は範囲内で、少し改善しています。
>
> フェリチンとTSAT(トランスフェリン飽和度)も範囲内で安定しており、
> TSATは前回から若干上昇しています。これは体内の鉄の状態を示しています。
>
> 診断・治療方針は必ず主治医にご確認ください。

グラフはそのまま、所見コメントは実際の生成結果をもとに一部表現を編集したもの。
...
- OCR・推論・保存はすべてローカルで完結する。検査データが外部へ出るのは、
  利用者自身のGoogle Driveへバックアップする場合のみ(`/backup`を呼ばなければ発生しない)
